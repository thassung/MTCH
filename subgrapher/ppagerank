...
def approximate_ppr(adj_matrix, seed_set, alpha=0.85, epsilon=1e-3, seed_weights=None):
    """
    ...
    :param seed_weights: list or numpy array, optional
        Custom weights for each seed node. If None, weights are distributed evenly.
        Must sum to 1.0 and have same length as seed_set.
    ...
    if seed_weights is not None:
        if len(seed_weights) != len(seed_set):
            raise ValueError("seed_weights must have same length as seed_set")
        if not np.isclose(np.sum(seed_weights), 1.0):
            raise ValueError("seed_weights must sum to 1.0")
        for seed, weight in zip(seed_set, seed_weights):
            res[seed] = weight
    else:
...
def multicom(adj_matrix, seedset, scoring, cut, explored_ratio=0.8, one_community=True, seed_weights=None):
    """
    ...
    :param one_community: bool, default True
        If True, treat all seeds as one set. If False, process each seed individually.
    :param seed_weights: list or numpy array, optional
        Custom weights for each seed node. Only used when one_community=True.
        Must sum to 1.0 and have same length as seedset.
    :return:
    seeds: list of int
    ...
    if (one_community):
        scores = scoring(adj_matrix, seedset, seed_weights=seed_weights)
