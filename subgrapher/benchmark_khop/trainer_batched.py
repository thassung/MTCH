...
class KHopSubgraphCache:
    """Pre-extracted subgraph topology for every training edge.

    Cache is shared across encoders for the same (dataset, k).
    """
    ...
    @classmethod
    def build(cls, source_edge, target_edge, data, khop_extractor, verbose=True):
        n = source_edge.size(0)
        sel_list, ei_list, u_list, v_list, nn_list = [], [], [], [], []
        skipped = 0

        it = tqdm(range(n), desc='Building k-hop cache', leave=False) if verbose else range(n)
        for i in it:
...
def build_or_load_khop_cache(source_edge, target_edge, data, khop_extractor,
                              cache_dir=None, verbose=True):
    if cache_dir:
        path = os.path.join(cache_dir, 'train_subgraphs.pt')
        if os.path.isfile(path):
            if verbose:
                print(f'[Cache] Loading k-hop subgraphs from {path}')
            return KHopSubgraphCache.load(path)

    if verbose:
        print('[Cache] Extracting k-hop subgraphs (one-time cost)...')
    cache = KHopSubgraphCache.build(source_edge, target_edge, data,
                                     khop_extractor, verbose)
    if cache_dir:
...
def train_epoch_khop_cached(encoder, predictor, data, cache, optimizer,
...
def train_model_khop_batched(encoder, predictor, data, split_edge, khop_extractor,
    ...
    cache = build_or_load_khop_cache(source_edge, target_edge, data,
                                      khop_extractor, cache_dir, verbose)
