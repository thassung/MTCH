...
"}\n"
"FIGW = 7  # base figure width in inches\n"
"\n"
"# ── Load results ──\n"
"RESULT_PATHS = [\n"
"    'results/benchmark-comparison/all_results.json',\n"
"]\n"
"\n"
"# Point this to an external directory containing full_results.json files\n"
"# from previous runs. Set to None to skip.\n"
"RESULT_DIR = '<result_dir>'  # <-- FILL THIS IN\n"
"\n"
"raw = []\n"
...
"        print(f'Loaded {rp}')\n"
"\n"
"# Scan RESULT_DIR for full_results.json\n"
"if RESULT_DIR and RESULT_DIR != '<result_dir>' and os.path.isdir(RESULT_DIR):\n"
"    for root, _, files in os.walk(RESULT_DIR):\n"
"        for fname in files:\n"
"            if fname == 'full_results.json':\n"
...
"                    method = 'External'\n"
"                    cfg, cfg_val = '-', None\n"
"                    if 'ppr_alpha' in r.get('config', {}):\n"
"                        method = 'Static PPR'\n"
"                        tk = r.get('top_k', r.get('config', {}).get('top_k', '?'))\n"
"                        cfg, cfg_val = f'top_k={tk}', tk\n"
"                    elif 'k' in r:\n"
"                        method = 'Static k-hop'\n"
"                        cfg, cfg_val = f'k={r[\"k\"]}', r['k']\n"
"                    elif 'teleport_values' in r:\n"
"                        method = 'Learnable PPR'\n"
"                        tk = r.get('top_k', '?')\n"
"                        cfg, cfg_val = f'top_k={tk}', tk\n"
"                    elif r.get('config', {}).get('batch_size', 0) > 50000:\n"
"                        method = 'Full Graph'\n"
"                    raw.append({\n"
"                        'method': method,\n"
"                        'dataset': r.get('dataset', '?'),\n"
"                        'encoder': r.get('encoder', r.get('model_name', '?')),\n"
"                        'config': cfg, 'config_value': cfg_val,\n"
...
"                        'train_time': float(r.get('train_time', 0)),\n"
"                    })\n"
"                    print(f'  External: {method} | {raw[-1][\"dataset\"]} | {raw[-1][\"encoder\"]}')\n"
...
"\n"
"if not df.empty:\n"
"    df.to_csv(os.path.join(save_dir, 'full_table.csv'), index=False)\n"
"    print(f'Saved full_table.csv ({len(df)} rows)')\n"
"\n"
"if not df_best.empty:\n"
"    df_best.to_csv(os.path.join(save_dir, 'best_config_table.csv'), index=False)\n"
"    print(f'Saved best_config_table.csv ({len(df_best)} rows)')\n"
"\n"
"    # Best per (dataset, encoder)\n"
"    winners = []\n"
...
"        os.path.join(save_dir, 'winners.csv'), index=False)\n"
