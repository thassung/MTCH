...
Static PPR uses LCILP-style approximate PPR + conductance sweep cut
(joint seed {u,v}, no per-node precomputation, adaptive subgraph size).
...
"| Static PPR | PPR + sweep cut | α=0.85, ε=1e-3 (LCILP) | `results/benchmark-ppr/{ds}/{enc}_a{a}_e{e}/` |\n"
"| Static k-hop | k-hop | k=2 | `results/benchmark-khop/{ds}/{enc}_k{k}/` |\n"
"\n"
...
"SUB_EPOCHS          = 500\n"
"SUB_BATCH_SIZE      = 1024      # subgraphs per GNN forward (increased for A6000 49 GB)\n"
"SUB_LR              = 0.005\n"
"SUB_PATIENCE        = 30\n"
...
"GRAD_CLIP           = 1.0\n"
"\n"
"# GAT-specific overrides: 1024-dim intermediates + ELU explode on tiny subgraphs\n"
"# at the default LR=0.005. Lower LR + tighter clip stabilises attention training.\n"
"ENCODER_LR_OVERRIDE   = {'GAT': 0.001}\n"
"ENCODER_CLIP_OVERRIDE = {'GAT': 0.5}\n"
"\n"
"# Static PPR — single config matching LCILP (alpha=0.85, epsilon=1e-3)\n"
"# LCILP does NOT sweep; one config is enough for a valid comparison.\n"
"PPR_ALPHAS    = [0.85]   # LCILP value\n"
"PPR_EPSILONS  = [1e-3]   # LCILP primary value (1e-4 gives larger subgraphs but slower)\n"
"PPR_WINDOW    = 10       # sweep cut early-stop window\n"
"DRNL_MAX_DIST = 6        # DRNL BFS distance bins (LCILP default)\n"
"DRNL_DIM      = 2 * (DRNL_MAX_DIST + 1)  # = 14 input features per node\n"
"PPR_MARGIN    = 10.0     # MarginRankingLoss margin (LCILP default)\n"
"\n"
"# Static k-hop — k=2 is the standard baseline; k=1 subgraphs are too small\n"
"K_VALUES = [2]\n"
"\n"
"import torch\n"
"# Set GPU_ID = 0 or 1 to target a specific card (useful when both A6000s are available)\n"
"GPU_ID = 0\n"
